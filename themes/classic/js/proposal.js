...
$(document).ready(function() {
  $('.participant-limit-popup-close').click(function() {
    $('#participant-limit-popup').modal('hide');
  });
  $('.openModal').click(function() {
    $('#opinion').children('.row').css('pointer-events', 'none');
    ...
    $.ajax({
      ...
      success: function(resp) {
        $('#opinion').children('.row').css('pointer-events', 'auto');
        ...
        if (resp.msg.opinion == 0) {
          ...
          for (var key in resp.msg.opinion) {
            if (resp.msg.opinion[key].content.description != '') {
              ...
              opinionBox += '<hr/>\n\
                             <div class="row ' + resp.msg.opinion[key].class + ' by-' + resp.msg.opinion[key].author.slug + '">\n\
  ...
  if (proposalSubmissionStatus == 0) {
    var discussionSlugForAlertPopUp = $.cookie('discussion');
    var popUpAlertDiscussions = [];
    if (discussionSlugForAlertPopUp != null) {
      popUpAlertDiscussions = discussionSlugForAlertPopUp.split(',');
    }
    if ($.inArray(discussionSlug, popUpAlertDiscussions) == -1) {
      $('#participant-limit-popup').modal('show');
      if (discussionSlugForAlertPopUp == null) {
        discussionSlugForAlertPopUp = discussionSlug;
      } else {
        discussionSlugForAlertPopUp = discussionSlugForAlertPopUp + ',' + discussionSlug;
      }
    }
    $.cookie('discussion', discussionSlugForAlertPopUp);
  }
});
...
function setOpinionComment() {
  ...
  if (commentInOpinionBox == 1 && typeof($('#opinion').children('.opinionbox').children('.row').find("strong").html()) == "undefined") {
    $('#opinion').children('.opinionbox').html("<hr>" + div);
...
function setOpinionCountAndHeatMap(opinion) {
  ...
  $('.active-parent').children('.proposal').children('.openModal').children('#proposal-heatmap-block').html(heatMap);
}
